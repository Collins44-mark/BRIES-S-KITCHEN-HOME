  ...
  return (
    <section className="glass-card flex h-full flex-col p-3.5 sm:p-[18px]">
      <div className="mb-3 flex items-center justify-between gap-2">
        <h3 className="text-[15px] font-semibold text-slate-900">{t('dashboard.topDebtors')}</h3>
        <Link href="/debts" className="view-all">
          {t('common.viewAll')}
        </Link>
      </div>
      {items.length === 0 ? (
        <p className="py-6 text-center text-sm text-slate-400">{t('dashboard.noDebtors')}</p>
      ) : (

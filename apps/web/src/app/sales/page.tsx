  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-card flex flex-col gap-3 p-4 sm:flex-row">
        ...
        >
          <option value="ALL">
            {t('common.all')} — {t('sales.paymentStatus')}
          </option>
        ...
        <div className="table-scroll">
          ...
          <thead className="bg-slate-50/70 text-xs uppercase text-slate-400">
            <tr>
              ...
              <th className="px-4 py-3">{t('common.paid')}</th>
              <th className="px-4 py-3">{t('pos.amountDue')}</th>
              <th className="px-4 py-3">{t('common.status')}</th>
              <th className="px-4 py-3">{t('sales.cashier')}</th>
          ...
          <tbody>
            ...
            {!isLoading && !isError && sales.length === 0 && (
              <TableEmptyRow colSpan={8} message={t('common.noDataPeriod')} />
            )}
            {!isLoading &&
              !isError &&
              sales.map((sale: SaleListItem) => {
                const payKey = statusKey(sale.paymentStatus);
                return (
                <tr
                  key={sale.id}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    >
                      {payKey ? t(payKey) : sale.paymentStatus}
                    </span>
                  </td>
      ...
      {selectedId && (
        <SaleDetailModal
          ...
          error={detailError}
          errorMessage={
            detailErrorObj instanceof Error ? detailErrorObj.message : t('common.unableLoad')
          }
...
function SaleDetailModal({
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      toast.success(t('sales.downloadPdf'));
    } catch (err) {
  ...
  const saleStatusLabelKey = detail ? statusKey(detail.status) : null;
  const paymentStatusLabelKey = detail ? statusKey(detail.paymentStatus) : null;
        ...
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-4 py-4 sm:px-5">
          <div className="min-w-0">
            ...
            <h2 id="sale-detail-title" className="truncate text-lg font-semibold text-slate-900">
              {detail?.invoiceNumber ?? (loading ? t('sales.loading') : t('sales.detail'))}
            </h2>
            {detail ? (
        ...
        <div className="overflow-y-auto px-4 py-5 sm:px-5">
          ...
          {detail && !loading && !error && (
            <div className="space-y-6">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-slate-100 bg-white/60 p-4">
                  ...
                  {detail.customerId ? (
                    ...
                    <p className="mt-1 font-medium text-slate-900">{t('pos.walkIn')}</p>
                  )}
                </div>
                  ...
                  <table className="w-full min-w-[560px] text-left text-sm">
                    <thead className="bg-slate-50/80 text-xs uppercase text-slate-400">
                      <tr>
                        ...
                        <th className="px-3 py-2.5 font-medium">{t('common.price')}</th>
                        <th className="px-3 py-2.5 font-medium">{t('common.subtotal')}</th>
                        <th className="px-3 py-2.5 font-medium">{t('common.discount')}</th>
                        <th className="px-3 py-2.5 font-medium">{t('common.total')}</th>
                        {showCostProfit ? (
                          <>
                            <th className="px-3 py-2.5 font-medium">{t('products.cost')}</th>
                            <th className="px-3 py-2.5 font-medium">{t('common.profit')}</th>
                          </>
                        ) : null}
                      </tr>
                    </thead>
              ...
              <div>
                <h3 className="mb-2 text-sm font-semibold text-slate-800">{t('sales.payments')}</h3>
                {detail.payments.length === 0 ? (
                  <p className="rounded-xl border border-slate-100 bg-white/60 px-4 py-5 text-center text-sm text-slate-400">
                    {t('common.noDataPeriod')}
                    {Number(detail.amountDue) > 0
                      ? ` · ${formatTzs(detail.amountDue)} ${t('debts.outstanding')}`
                      : ''}
                  </p>
                ) : (
                  <ul className="space-y-2">
                    {detail.payments.map((p) => {
                      const methodKey = paymentMethodKey(p.method);
                      return (
                      <li
                        key={p.id}
                      ...
                      >
                        <div className="min-w-0">
                          <p className="font-medium text-slate-800">
                            {methodKey.startsWith('common.') ? t(methodKey) : p.method}
                          </p>
                ...
                {Number(detail.amountDue) > 0 && detail.payments.length > 0 ? (
                  <p className="mt-2 text-xs text-slate-500">
                    {t('debts.outstanding')}: {formatTzs(detail.amountDue)}
                  </p>
                ) : null}
              </div>
              ...
              <div className="flex flex-col-reverse gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:flex-wrap sm:justify-end">
                ...
                >
                  ...
                  {pdfBusy ? t('common.loading') : t('sales.downloadPdf')}
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  {printBusy ? t('common.loading') : t('sales.printReceipt')}
                </button>
              </div>
            </div>
          )}

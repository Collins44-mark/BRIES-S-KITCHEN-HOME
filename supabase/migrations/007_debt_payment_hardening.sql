-- =============================================================================
-- 007: Debt payment & create_sale hardening (Phase 5B)
--
-- 1) create_sale: reject method = CREDIT on payment lines (defense in depth).
--    Unpaid balance remains sales.amount_due — never a CREDIT payment row.
--    Behavior otherwise identical to 006 (multi-unit sales).
--
-- 2) record_debt_payment: require sale_id; only COMPLETED sales;
--    reject cancelled/refunded/non-completed; keep locks + overpay guards.
-- =============================================================================
...
DECLARE
  ...
  locked_unit public.product_units;
  item jsonb;
  v_selling_quantity integer;
  v_product_id uuid;
  v_product_unit_id uuid;
  v_base_quantity integer;
  v_required_base integer;
  resolved_unit_price numeric(18, 2);
  tier_price numeric(18, 2);
...
BEGIN
  ...
  -- Aggregate REQUIRED BASE quantity per product (multi-unit safe)
  FOR pid, v_required_base IN
    SELECT
      (elem ->> 'product_id')::uuid AS v_product_id,
      SUM(
        (elem ->> 'quantity')::integer
        * pu.conversion_to_base
      )::integer AS v_required_base
    FROM jsonb_array_elements(items) AS elem
    INNER JOIN public.product_units pu
      ON pu.id = NULLIF(elem ->> 'product_unit_id', '')::uuid
     AND pu.product_id = (elem ->> 'product_id')::uuid
  ...
  LOOP
    ...
    IF locked_product.stock_quantity < v_required_base THEN
      RAISE EXCEPTION 'Insufficient stock for %', locked_product.name;
    END IF;
  END LOOP;

  -- Ensure every line had a valid active unit (aggregation join would drop bad lines)
  IF (
    SELECT COUNT(*)
    FROM jsonb_array_elements(items) AS elem
  ) <> (
    ...
    -- Identify first bad line for a clear error
    FOR item IN SELECT * FROM jsonb_array_elements(items)
    LOOP
      v_product_id := NULLIF(item ->> 'product_id', '')::uuid;
      v_product_unit_id := NULLIF(item ->> 'product_unit_id', '')::uuid;
      v_selling_quantity := (item ->> 'quantity')::integer;

      IF v_product_id IS NULL OR v_product_unit_id IS NULL OR v_selling_quantity IS NULL OR v_selling_quantity < 1 THEN
        RAISE EXCEPTION 'Each sale item needs product_id, product_unit_id, and quantity >= 1';
      END IF;
      ...
      IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Invalid or inactive selling unit for product %', v_product_id;
      END IF;
    END LOOP;

    RAISE EXCEPTION 'Invalid or inactive selling unit';
  END IF;

  -- Build calculated lines (server-authoritative price + base quantity)
  FOR item IN SELECT * FROM jsonb_array_elements(items)
  LOOP
    v_product_id := NULLIF(item ->> 'product_id', '')::uuid;
    v_product_unit_id := NULLIF(item ->> 'product_unit_id', '')::uuid;
    ...
    SELECT * INTO locked_unit
    FROM public.product_units pu
    WHERE pu.id = v_product_unit_id
      AND pu.product_id = v_product_id
      AND pu.is_active = true;

    IF locked_unit.id IS NULL THEN
      RAISE EXCEPTION 'Invalid or inactive selling unit for product %', locked_product.name;
    END IF;

    -- Retail from unit; optional wholesale: highest active min_quantity <= selling qty
    resolved_unit_price := locked_unit.selling_price;

    SELECT ppt.unit_price
    INTO tier_price
    FROM public.product_price_tiers ppt
    WHERE ppt.product_unit_id = locked_unit.id
      AND ppt.is_active = true
      AND ppt.min_quantity <= v_selling_quantity
    ORDER BY ppt.min_quantity DESC
    LIMIT 1;

    IF FOUND THEN
      resolved_unit_price := tier_price;
    END IF;

    v_base_quantity := v_selling_quantity * locked_unit.conversion_to_base;
    IF v_base_quantity < 1 THEN
      RAISE EXCEPTION 'Invalid base quantity for %', locked_product.name;
    END IF;

    line_subtotal := public.money_round(resolved_unit_price * v_selling_quantity);
    -- Cost always on BASE units
    line_cost := public.money_round(locked_product.cost_price * v_base_quantity);
    ...
    calc_items := calc_items || jsonb_build_array(
      jsonb_build_object(
        ...
        'selling_unit_code', locked_unit.unit_code,
        'selling_unit_label', COALESCE(NULLIF(locked_unit.unit_label, ''), locked_unit.unit_code::text),
        'conversion_to_base', locked_unit.conversion_to_base,
        'quantity', v_selling_quantity,
  ...
  IF jsonb_typeof(payments) = 'array' THEN
    ...
    LOOP
      ...
      -- Defense in depth: unpaid remainder is amount_due, never a CREDIT row.
      IF upper(COALESCE(payment ->> 'method', '')) = 'CREDIT' THEN
        RAISE EXCEPTION
          'CREDIT cannot be used as a payment line. Leave the unpaid amount as debt.';
      END IF;
  ...
  -- Sale items + stock + inventory movements (BASE units)
  FOR idx IN 0 .. item_count - 1 LOOP
    line_rec := calc_items -> idx;
    v_product_id := (line_rec ->> 'product_id')::uuid;
    v_selling_quantity := (line_rec ->> 'quantity')::integer;
    v_base_quantity := (line_rec ->> 'base_quantity')::integer;
    ...
    VALUES (
      ...
      (line_rec ->> 'product_unit_id')::uuid,
      (line_rec ->> 'selling_unit_code')::public.product_unit_code,
      line_rec ->> 'selling_unit_label',
      (line_rec ->> 'conversion_to_base')::integer,
      v_base_quantity
    );
    ...
    qty_after := qty_before - v_base_quantity;
    IF qty_after < 0 THEN
  ...
  -- Payment rows (money methods only — CASH / MPESA / BANK)
  IF jsonb_typeof(payments) = 'array' THEN
    FOR payment IN SELECT * FROM jsonb_array_elements(payments)
    LOOP
      pay_amount := public.money_round((payment ->> 'amount')::numeric);
      ...
      END IF;
      IF pay_method = 'CREDIT'::public.payment_method THEN
        RAISE EXCEPTION
          'CREDIT cannot be used as a payment line. Leave the unpaid amount as debt.';
      END IF;
...
COMMENT ON FUNCTION public.create_sale(jsonb) IS
  'Completes a sale. Items require product_unit_id; quantity is selling-unit qty; '
  'stock deducts base_quantity = qty * conversion_to_base. Prices from product_units '
  '+ highest matching active price tier. Rejects CREDIT payment lines — unpaid '
  'remainder is sales.amount_due.';

-- =============================================================================
-- record_debt_payment hardening
-- - sale_id REQUIRED (no account-only payments)
-- - sale must be COMPLETED
-- - CREDIT method rejected
-- - overpayment rejected
-- - FOR UPDATE locks retained
-- =============================================================================
...
BEGIN
  ...
  IF v_sale_id IS NULL THEN
    RAISE EXCEPTION 'sale_id is required. Debt payments must target a specific sale.';
  END IF;
  ...
  -- Validate sale before any balance mutation (locks row; raises roll back atomically).
  SELECT * INTO sale
  FROM public.sales s
  WHERE s.id = v_sale_id
  FOR UPDATE;
  ...
  IF sale.status IS DISTINCT FROM 'COMPLETED'::public.sale_status THEN
    RAISE EXCEPTION
      'Debt payments are only allowed for COMPLETED sales.';
  END IF;
...
COMMENT ON FUNCTION public.record_debt_payment(jsonb) IS
  'Records a debt repayment against a COMPLETED sale. sale_id is required. '
  'Rejects CREDIT method, overpayment, and non-completed sales. Atomically '
  'updates payments, sales, customer_accounts, customer_transactions, audit_logs.';

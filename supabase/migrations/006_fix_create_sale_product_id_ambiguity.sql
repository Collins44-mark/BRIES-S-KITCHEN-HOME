-- =============================================================================
-- 006: Fix create_sale product_id PL/pgSQL variable vs column ambiguity
--
-- Root cause (005): local variables named product_id / product_unit_id collided
-- with table columns of the same name under PL/pgSQL #variable_conflict (error).
-- PostgreSQL error: column reference "product_id" is ambiguous
-- Detail: It could refer to either a PL/pgSQL variable or a table column.
--
-- Exact failing pattern in 005:
--   FROM public.product_units pu
--   WHERE pu.id = product_unit_id
--     AND pu.product_id = product_id   -- RHS product_id is ambiguous
--
-- Fix: rename locals to v_product_id / v_product_unit_id / v_selling_quantity /
-- v_base_quantity / v_sale_id and keep table columns qualified (pu.product_id).
-- Behavior otherwise identical to 005 (multi-unit sales). Function-only change.
-- =============================================================================
...
COMMENT ON FUNCTION public.create_sale(jsonb) IS
  ...
  '+ highest matching active price tier. No CREDIT payment lines.';

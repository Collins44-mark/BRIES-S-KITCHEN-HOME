-- =============================================================================
-- 005: Multi-unit create_sale (selling units + wholesale tiers)
--
-- Breaking change vs 002:
--   Each item MUST include product_unit_id (no silent PCS fallback).
--   quantity = SELLING UNIT quantity (not base stock units).
--   Stock checks / deductions use base_quantity = qty * conversion_to_base.
--   unit_price resolved server-side from product_units + product_price_tiers.
--
-- Preserved from 002:
--   SECURITY DEFINER, staff roles, invoice next_invoice_number(),
--   payments (no CREDIT lines), customer ledger, discount allocation,
--   product FOR UPDATE locking in sorted uuid order, walk-in fully paid.
--
-- Does NOT modify purchases, receive_purchase, reports, or unit config tables.
-- Requires migration 004 columns on sale_items (already applied in production).
-- =============================================================================
...
BEGIN
  ...
  FOR pid, required_base IN
    SELECT
      (elem ->> 'product_id')::uuid AS product_id,
      SUM(
        (elem ->> 'quantity')::integer
        * pu.conversion_to_base
      )::integer AS required_base
    FROM jsonb_array_elements(items) AS elem
  ...
  LOOP
    ...
    IF locked_product.stock_quantity < required_base THEN
  ...
  IF (
    ...
    LOOP
      ...
      product_unit_id := NULLIF(item ->> 'product_unit_id', '')::uuid;
      qty := (item ->> 'quantity')::integer;

      IF product_id IS NULL OR product_unit_id IS NULL OR qty IS NULL OR qty < 1 THEN
      ...
      IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Invalid or inactive selling unit for product %', product_id;
      END IF;
    END LOOP;
  ...
  -- Build calculated lines (server-authoritative price + base qty)
  FOR item IN SELECT * FROM jsonb_array_elements(items)
  LOOP
    product_id := NULLIF(item ->> 'product_id', '')::uuid;
    product_unit_id := NULLIF(item ->> 'product_unit_id', '')::uuid;
    ...
    base_qty := qty * locked_unit.conversion_to_base;
    IF base_qty < 1 THEN
      RAISE EXCEPTION 'Invalid base quantity for %', locked_product.name;
    END IF;

    line_subtotal := public.money_round(resolved_unit_price * qty);
    -- Cost always on BASE units
    line_cost := public.money_round(locked_product.cost_price * base_qty);
  ...
  FOR idx IN 0 .. item_count - 1 LOOP
    ...
    base_qty := (line_rec ->> 'base_quantity')::integer;
